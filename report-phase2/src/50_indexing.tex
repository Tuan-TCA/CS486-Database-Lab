...
The tuning folder also contains a read-only index-inventory diagnostic. It lists index type, uniqueness, primary-key status, and unique-constraint status for the five workload tables, making the baseline access structures explicit before interpreting scan and seek behavior. The updated benchmark harness otherwise preserves the measured workloads and result methodology used below.
