...
- **USAGE_SESSION** (**session_id**, _booking_id_, actual*start_time, actual_end_time, \_checked_in_by_user_id*, _completed_by_user_id_, initial_condition, final_condition, usage_notes)
...
| building       | VARCHAR   | Not Null, **Candidate Key (1/2)** | Building name or code                     |
| floor          | INT       | Not Null                          | Floor number                              |
| room_number    | VARCHAR   | Not Null, **Candidate Key (2/2)** | Room number (Unique within a building)    |
...
| _booking_id_         | VARCHAR   | **Foreign Key**, **Candidate Key** | References `BOOKING_REQUEST(booking_id)`. **UNIQUE** constraint enforces the 1:1 relationship. |
