# Skill 02: Conceptual Database Design (ERD) (Round 2 Snapshot)
...
4. **Attribute Listing:** List all attributes under each entity in the Mermaid diagram. Annotate Primary Keys (`PK`) and Foreign Keys (`FK`). **Do NOT include data types (e.g., `string`, `int`).**
5. **Relationship Mapping:** Map every relationship explicitly listed in Section 3.5 of `result_round3.md`.
6. **Participation Constraints:** Determine and visualize mandatory vs. optional participation constraints.
7. **Diagram Clarity:** Keep the diagram clean and well-structured.
8. **Assumption Documentation:** If any participation constraint is inferred, document it beneath the diagram in an Assumptions list.
...
3. Count the relationships in the ERD to ensure there are exactly 11.
4. Verify that the Mermaid syntax compiles correctly.
