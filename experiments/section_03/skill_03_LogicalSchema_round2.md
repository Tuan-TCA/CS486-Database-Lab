# Skill 03: Logical Schema Design (Round 2 Snapshot)
...
4. **Primary and Foreign Keys:** Explicitly define `PRIMARY KEY` and `FOREIGN KEY` constraints. **CRITICAL: All Foreign Keys MUST explicitly define `ON DELETE RESTRICT` (or equivalent) to satisfy the "No hard deletes" requirement (Rule 10).**
...
5. Verify that no new attributes or tables were invented.
