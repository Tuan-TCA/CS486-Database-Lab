...
-- Schema Reference: USER, SPACE, FACILITY, BOOKING_REQUEST, BOOKING_APPROVAL, USAGE_SESSION, MAINTENANCE_RECORD
-- Temporal Anchor: The sample data uses static 2026 dates, so GETDATE() is replaced with a static @ReportDate variable to ensure query results are reproducible.
-- ============================================================
...
DECLARE @ReportDate DATETIME = '2026-06-30 20:00:00';
SELECT booking_id, requested_start_time, s.space_name
FROM BOOKING_REQUEST br
JOIN SPACE s ON br.space_code = s.space_code
WHERE br.status = 'Approved' AND br.requested_start_time > @ReportDate; 
...
FROM SPACE
WHERE space_type = 'Classroom' AND current_status != 'Under Maintenance';
...
-- Query 13: Booking History (Fixed with LEFT JOIN)
-- Business Question: Total bookings per student?
-- Target User(s): Admin
-- Explanation: Engagement tracking that includes users with 0 bookings.
SELECT u.user_id, COUNT(br.booking_id) AS total_bookings
FROM [USER] u
LEFT JOIN BOOKING_REQUEST br ON u.user_id = br.user_id 
...
FROM BOOKING_REQUEST JOIN SPACE ON BOOKING_REQUEST.space_code = SPACE.space_code;
