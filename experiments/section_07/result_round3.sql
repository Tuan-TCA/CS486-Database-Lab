-- ============================================================
-- Section 07: Query Design — Round 3 (Final)
...
-- Index: Composite index on MAINTENANCE_RECORD(space_code, status) improves active maintenance filter.
...
-- Index: Composite index on MAINTENANCE_RECORD(status, start_time) improves active maintenance filtering.
SELECT
    S.space_code,
    S.space_name,
    S.building,
...
-- Q15 — Top 3 Users with Most Completed Bookings (TOP WITH TIES)
-- Business Question: Who are the top 3 users with the most completed bookings?
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Identifies power users for recognition or capacity planning; TOP WITH TIES ensures ties are included.
SELECT TOP 3 WITH TIES
    U.user_id,
    U.full_name,
    U.role,
    U.department,
    COUNT(BR.booking_id) AS completed_bookings
FROM [USER] U
LEFT JOIN BOOKING_REQUEST BR ON BR.user_id = U.user_id AND BR.status = 'Completed'
GROUP BY U.user_id, U.full_name, U.role, U.department
...
-- Target User(s): Facility Manager, Facility Staff
-- Explanation: Flags overdue pending requests. Uses requested_start_time as a proxy for submission time because no created_at column exists in the schema.
SELECT
    BR.booking_id,
    U.full_name AS requester,
    U.role,
...
-- Index: Composite index on FACILITY(space_code, facility_name) improves facility-space joins.
SELECT
    F.facility_name,
