...
WHERE br.status = 'Approved' AND br.requested_start_time > GETDATE();
...
FROM SPACE
WHERE space_type = 'Classroom' AND status != 'Under Maintenance'; -- BUG: Hallucinated column 'status'
...
SELECT space_code FROM SPACE WHERE status = 'In Use'; -- BUG: status instead of current_status
...
SELECT u.user_id, COUNT(br.booking_id) AS no_shows
FROM [USER] u
JOIN BOOKING_REQUEST br ON u.user_id = br.user_id
WHERE br.status = 'No-show'
GROUP BY u.user_id
ORDER BY no_shows DESC
LIMIT 3; -- BUG: Postgres syntax LIMIT
...
SELECT space_code FROM SPACE WHERE status = 'Under Maintenance';
...
FROM [USER] u
INNER JOIN BOOKING_REQUEST br ON u.user_id = br.user_id -- BUG: INNER JOIN drops zero bookings
...
SELECT user_id, SUM(CASE WHEN status = 'Rejected' THEN 1 ELSE 0 END) / COUNT(booking_id) AS rate
FROM BOOKING_REQUEST
GROUP BY user_id; -- BUG: Division by zero risk
...
SELECT AVG(expected_participants) / AVG(capacity) 
FROM BOOKING_REQUEST JOIN SPACE ON BOOKING_REQUEST.space_code = SPACE.space_code; -- BUG: Div zero
...
SELECT session_id FROM USAGE_SESSION WHERE initial_condition = 'Good' AND final_condition = 'Bad';
