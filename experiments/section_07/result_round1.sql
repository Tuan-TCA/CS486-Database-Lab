-- ============================================================
-- Section 07: Query Design — Round 1
...
-- Explanation: Helps staff prioritize which requests need review, ensuring timely responses to booking requests.
SELECT
    BR.booking_id,
...
-- Explanation: Allows staff to prepare spaces for upcoming approved events and anticipate facility usage.
SELECT
    BR.booking_id,
    U.full_name AS requester,
    S.space_name,
...
-- Explanation: Enables users and staff to review past, current, and future bookings for a given individual.
SELECT
    BR.booking_id,
    S.space_name,
    BR.requested_start_time,
...
-- Explanation: Identifies scheduling conflicts that slipped through, so staff can resolve them proactively.
SELECT
...
-- Explanation: Reveals underutilized spaces that could be repurposed or promoted for more frequent use.
SELECT
    S.space_code,
    S.space_name,
    S.space_type,
...
-- Business Question: Which bookings are currently checked in and in progress right now?
-- Target User(s): Facility Staff, Facility Manager
-- Explanation: Provides a real-time view of which spaces are occupied, helping staff manage walk-in requests and monitor space usage.
SELECT
    BR.booking_id,
    U.full_name AS requester,
    S.space_name,
...
-- Target User(s): Facility Staff, Department Administrator
-- Explanation: Helps identify whether sessions are overrunning or finishing early, informing scheduling policies.
SELECT
    BR.booking_id,
    S.space_name,
    U.full_name AS requester,
...
-- Q9 — No-Show Bookings
-- Business Question: Which approved bookings resulted in no-shows (user never checked in)?
-- Target User(s): Facility Manager, Facility Staff
-- Explanation: Tracks wasted reservations so managers can follow up with users and consider penalty policies.
SELECT
    BR.booking_id,
    U.full_name AS requester,
    U.email,
...
-- Target User(s): Facility Staff, Facility Manager
-- Explanation: Provides a snapshot of spaces that are unavailable due to maintenance issues.
SELECT
    S.space_code,
    S.space_name,
    S.building,
...
-- Target User(s): Facility Manager, Facility Staff
-- Explanation: Helps track recurring issues and assess whether a space requires long-term attention.
SELECT
    MR.maintenance_id,
    MR.problem_description,
    MR.start_time,
...
-- Business Question: Which bookable spaces are currently blocked because of unresolved maintenance?
-- Target User(s): All Users
-- Explanation: Prevents users from attempting to book spaces that are unavailable due to active maintenance.
SELECT
    S.space_code,
    S.space_name,
    S.space_type,
...
-- Q13 — Most Active Users by Booking Count
-- Business Question: Which users have made the most bookings overall?
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Identifies the most frequent space users for capacity planning and community recognition.
...
-- Business Question: What is the complete timeline of a specific user's bookings with all related details?
-- Target User(s): Facility Staff, User
-- Explanation: Provides a comprehensive audit trail of a user's interactions with the booking system.
SELECT
    BR.booking_id,
    S.space_name,
    S.building,
...
-- Business Question: Which users currently have pending booking requests, grouped by their role?
-- Target User(s): Facility Staff, Facility Manager
-- Explanation: Helps prioritize approvals based on user roles (e.g., lecturers may need faster processing).
SELECT
    U.role,
    U.full_name,
    U.email,
...
-- Business Question: Which pending bookings have been waiting the longest for an approval decision?
-- Target User(s): Facility Manager, Facility Staff
-- Explanation: Flags overdue pending requests so managers can enforce service-level targets.
SELECT
    BR.booking_id,
    U.full_name AS requester,
    U.role,
...
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Helps management understand patterns in rejections and potentially adjust policies to accommodate common requests.
SELECT
    BA.rejection_reason,
...
-- Target User(s): Facility Manager
-- Explanation: Monitors decision-making patterns of staff members who handle approvals.
SELECT
...
-- Q19 — Occupancy Rate by Space Type
-- Business Question: What percentage of total bookings does each space type account for?
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Reveals which types of spaces are in highest demand, informing future facility investment decisions.
SELECT
    S.space_type,
    COUNT(BR.booking_id) AS total_bookings,
    ROUND(
...
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Tracks seasonal demand patterns to help with resource allocation and staffing decisions.
SELECT
...
-- Target User(s): Facility Manager
-- Explanation: Identifies peak demand hours to optimize opening hours and staffing schedules.
SELECT
...
-- Business Question: Which facilities (equipment) are available in the most-booked spaces?
-- Target User(s): Facility Manager, Facility Staff
-- Explanation: Helps correlate facility availability with booking demand to identify high-value equipment.
SELECT
    F.facility_name,
...
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Helps set appropriate time slot templates and understand typical usage patterns.
SELECT
    BR.booking_type,
    COUNT(BR.booking_id) AS total_bookings,
...
-- Business Question: How does the actual number of expected participants compare to each space's maximum capacity?
-- Target User(s): Facility Manager, Department Administrator
-- Explanation: Identifies whether spaces are being overbooked or underutilized relative to their capacity.
SELECT
    S.space_name,
    S.space_type,
    S.capacity,
