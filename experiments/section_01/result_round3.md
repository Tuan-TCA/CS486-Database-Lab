...
## 3.5 Relationships & Cardinalities
- **USER (1) to BOOKING_REQUEST (N)**: A user can submit multiple booking requests.
- **SPACE (1) to BOOKING_REQUEST (N)**: A space can have multiple booking requests.
- **SPACE (1) to FACILITY (N)**: A space contains multiple facilities.
- **SPACE (1) to MAINTENANCE_RECORD (N)**: A space can have multiple maintenance records.
- **USER (1) to BOOKING_APPROVAL (N)**: A user (Facility Staff/Manager) can make multiple booking approvals via `decided_by_user_id`.
- **USER (1) to USAGE_SESSION (N) - Check In**: A user (Facility Staff) can check in multiple usage sessions via `checked_in_by_user_id`.
- **USER (1) to USAGE_SESSION (N) - Completion**: A user (Facility Staff) can complete multiple usage sessions via `completed_by_user_id`.
- **USER (1) to MAINTENANCE_RECORD (N) - Reporter**: A user can report multiple maintenance issues via `reporter_user_id`.
- **USER (1) to MAINTENANCE_RECORD (N) - Assignee**: A user (Facility Staff) can be assigned to multiple maintenance records via `assigned_staff_user_id`.
- **BOOKING_REQUEST (1) to BOOKING_APPROVAL (0..1)**: A booking request has at most one approval record (1:1 relationship).
- **BOOKING_REQUEST (1) to USAGE_SESSION (0..1)**: A booking request has at most one usage session (1:1 relationship).
...
### 4.8 Inferred Values
The following domain values are not explicitly stated in the project description but are inferred for completeness. These are documented further in the Assumptions section.
- **Account Statuses**: Active, Inactive, Suspended
- **Maintenance Statuses**: Reported, In Progress, Completed
...
The following matrix summarizes which actors are authorized to perform key system actions, based on the actor descriptions in the project description. Note: Facility Manager has been removed from operational actions like Check-in and Complete in alignment with strict prompt roles.
...
### 8.1 Space Status Lifecycle
The following states define the overarching status of a physical space:
...
```
*(Note: Terminal states Rejected, Cancelled, No-show, and Completed have no downstream transitions.)*
...
5. **Approved → Checked In:** At the scheduled time, staff performs check-in. A USAGE_SESSION record is created with `actual_start_time`, `checked_in_by_user_id`, and `initial_condition` (Rule 8). The space status changes to `In Use`.
...
8. **Checked In → Completed:** The session ends. Staff records `actual_end_time`, `completed_by_user_id`, `final_condition`, and `usage_notes` in the USAGE_SESSION (Rule 9). The space status reverts to `Available`.
...
1. **Report → Reported:** Any user reports a maintenance problem for a space. A MAINTENANCE_RECORD is created with the `reporter_user_id`, `space_code`, and `problem_description`. The space status changes to `Under Maintenance`.
...
4. **In Progress → Completed:** The maintenance work is finished. The `completion_time` and `result_note` are recorded. The space returns to `Available` status.
...
| 10 | "Account Status" tracks whether a user's account is active or inactive, supporting soft-delete of users. | Combined with Rule 10 (no hard deletes), inactive accounts are preserved for historical reference. **The exact enumerated values (Active, Inactive, Suspended) are inferred.** |
| 11 | Maintenance status tracks the progress of repairs. | **The exact enumerated values (Reported, In Progress, Completed) are inferred.** |
| 12 | Capacity check constraint: `expected_participants <= capacity` | Implicitly required to prevent booking requests from exceeding a room's physical limits, though not explicitly listed as a numbered business rule. |
...
| 6 | Explicit Relationships & Cardinalities mapped? | ✅ PASS (Section 3.5) |
| 7 | All domain value enumerations listed (Space Types, Space Statuses, Booking Types, Booking Statuses, Facility Examples, Maintenance Problem Types)? | ✅ PASS (Section 4) |
| 8 | Inferred values explicitly enumerated and declared as assumptions? | ✅ PASS (Sections 4.8 & 9) |
| 9 | Staff view/reporting requirements captured? | ✅ PASS (Section 7) |
| 10 | Booking and maintenance lifecycle workflows described with CANCELLED state and clean ASCII diagram? | ✅ PASS (Section 8) |
| 11 | Role-Action matrix strictly aligned with prompt? | ✅ PASS (Section 6) |
| 12 | Assumptions documented including capacity rule? | ✅ PASS (Section 9) |
| 13 | Scope boundaries defined? | ✅ PASS (Section 10) |
| 14 | Business rules categorized (not flat-listed)? | ✅ PASS (Section 5) |
